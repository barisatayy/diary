document.addEventListener("DOMContentLoaded", () => {
  ...
  const reminderDisplayContainer = document.getElementById(
  ...
  function renderCalendar() {
    ...
    monthYearHeader.textContent = calendarDate.toLocaleDateString("tr-TR", {
      month: "long",
      year: "numeric",
    });
    ...
    for (let day = 1; day <= lastDayOfMonth; day++) {
      ...
      if (allNotes[fullDateStr]?.notes.length > 0)
        dayDiv.classList.add("has-note");
      if (allReminders[fullDateStr]?.length > 0)
  ...
  function handleDayClick(dateStr) {
    if (reminderText.value.trim() !== "" || selectedReminderDates.length > 0) {
      if (dateStr <= todayDateStr) {
        showModal(
          "Sadece gelecek günler için hatırlatıcı kurabilirsiniz.",
          "alert"
        );
        return;
      }
  ...
  function renderNotesForDate(dateString) {
    ...
    dayData.notes.forEach((note) => createNoteElement(note, !isToday));
    if (!isToday) {
    ...
    if (dayData.notes.length === 0) {
      ...
      if (dateString > todayDateStr)
        message = "Gelecek için henüz not eklenemez.";
  ...
  function createNoteElement(note, isReadOnly) {
    ...
    if (!isReadOnly) {
      noteContent.addEventListener("input", () =>
        updateNoteText(note.id, noteContent.value)
      );
  ...
  function addNote() {
    ...
    const allNotes = loadData(DBNames.NOTES);
    if (!allNotes[currentViewDateStr]) {
      allNotes[currentViewDateStr] = { notes: [] };
    }
  ...
  function deleteNote(id) {
    showModal(
      "Bu notu kalıcı olarak silmek istediğinizden emin misiniz?",
      "confirm",
      (confirmed) => {
        if (confirmed) {
          _performDelete(id);
  ...
  function _performDelete(id) {
    ...
    if (!dayNotes) return;
    allNotes[currentViewDateStr].notes = dayNotes.filter(
      (note) => note.id !== id
    );
    saveData(DBNames.NOTES, allNotes);
  ...
  function setReminder() {
    ...
    if (text === "") {
      showModal("Lütfen bir hatırlatıcı metni girin.", "alert");
      return;
    }
    if (selectedReminderDates.length === 0) {
      showModal("Lütfen takvimden en az bir gelecek tarih seçin.", "alert");
      return;
    }
    const allReminders = loadData(DBNames.REMINDERS);
    selectedReminderDates.forEach((dateStr) => {
      if (!allReminders[dateStr]) allReminders[dateStr] = [];
      allReminders[dateStr].push(text);
    });
